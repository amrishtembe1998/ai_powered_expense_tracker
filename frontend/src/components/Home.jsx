...
const Home = () => {
    ...
    useEffect(() => {
        async function updateExpense(editExpense) {
            ...
            if (!data) {
                setError(
                    "Something wrong in updating the data. Please try again",
                );
            }
            setIsLoading(false);
    ...
    const onAddExpenseClickHandler = async (event) => {
        ...
        try {
            ...
            console.error(
                `Error has occured while calling addExpense API: ${error}`,
            );
        }
    };
    if (isLoading) {
    ...
    return (
        <div>
            <div className="mb-8 bg-gray-900">
                <div className="flex justify-center text-5xl pt-6">
                    Expense Tracker
                </div>
                <form
                    onSubmit={(event) => {
                ...
                {error && (
                    <h3 className="text-red-300 flex justify-center items-center. mt-2">
                        {error}
                    </h3>
                )}
                {expenses.length === 0 ? (
                    <h1 className="flex justify-center h-screen mt-8 text-4xl">
